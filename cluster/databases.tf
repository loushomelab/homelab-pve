...
provider "postgresql" {
  alias    = "auth"
  host     = data.doppler_secrets.db.map.DB_AUTH_POSTGRESQL__HOST
  port     = 5432
  database = "postgres"
  username = "postgres"
  password = data.doppler_secrets.db.map.DB_AUTH_POSTGRESQL__PASSWORD
  sslmode  = "disable"
}
...
provider "postgresql" {
  alias    = "obs"
  host     = data.doppler_secrets.db.map.DB_OBS_POSTGRESQL__HOST
  port     = 5432
  database = "postgres"
  username = "postgres"
  password = data.doppler_secrets.db.map.DB_OBS_POSTGRESQL__PASSWORD
  sslmode  = "disable"
}

resource "postgresql_role" "grafana" {
  provider = postgresql.obs
  name     = data.doppler_secrets.db.map.GRAFANA_POSTGRESQL__USER
  login    = true
  password = data.doppler_secrets.db.map.GRAFANA_POSTGRESQL__PASSWORD
}

resource "postgresql_database" "grafana" {
  provider = postgresql.obs
  name     = data.doppler_secrets.db.map.GRAFANA_POSTGRESQL__NAME
  owner    = postgresql_role.grafana.name
}

resource "postgresql_role" "umami" {
  provider = postgresql.obs
  name     = data.doppler_secrets.db.map.UMAMI_POSTGRESQL__USER
  login    = true
  password = data.doppler_secrets.db.map.UMAMI_POSTGRESQL__PASSWORD
}

resource "postgresql_database" "umami" {
  provider = postgresql.obs
  name     = data.doppler_secrets.db.map.UMAMI_POSTGRESQL__NAME
